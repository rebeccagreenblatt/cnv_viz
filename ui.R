...
fluidPage(
  ...
  navbarPage("CNViz",
             ...
             tabPanel(icon("info-circle", class = NULL, lib = "font-awesome"), fluid=TRUE,
                      ...
                      p("Unadjusted data is obtained from CNVkit. CNVkit takes hybrid capture DNA sequencing data and infers copy number. This tab also includes individual probe data for each gene."),
                      ...
                      p("Adjusted data is obtained from PureCN, using data generated by CNVkit. PureCN estimates tumor purity, tumor ploidy and loss of heterozygosity (LOH); copy number estimates are adjusted accordingly. Somatic mutations are also inferred. Of note, germline mutations are not displayed. Tumor purity is PureCN's estimate for the percentage of the sample containing tumor cells. Ploidy is estimated by PureCN, then rounded to the nearest whole number. Only segments where copy number differs from tumor ploidy or with LOH are highlighted. If a sample is marked as Flagged, this means PureCN has deemed the results as unreliable."),
